...
const OrdersModule = () => {
  ...
  const loadOrders = async () => {
    try {
      ...
      const response = await paymentsAPI.getPayments(params); // Reutilizando paymentsAPI por ahora
      ...
      setError(error.response?.data?.message || 'Error cargando órdenes');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-2">
            Monitorea y gestiona todas las órdenes de tu negocio
          </p>
        </div>
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              ...
              {orders.map((order, index) => (
                <div key={order.order_id || index} className="grid grid-cols-12 gap-4 p-3 bg-white rounded-lg border border-gray-200 hover:shadow-md transition-shadow">
                  <div className="col-span-3">
                    <p className="font-medium text-gray-800 text-sm">
                      {order.order_id || `ord_${index}`}
                    </p>
                    <p className="text-xs text-gray-500">
                      {order.customer_email || 'cliente@ejemplo.com'}
                    </p>
                  </div>
                  ...
                  <div className="col-span-2">
                    <div className="flex items-center space-x-1 text-sm text-gray-600">
                      ...
                      <span>{formatDate(order.created || new Date().toISOString())}</span>
                    </div>
                  </div>
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-600">
                Las órdenes aparecerán aquí cuando se procesen transacciones
              </p>
            </div>
          )}
        </CardContent>
